...
public function export_summary($id, $token)
{
	...
	if($rh) //// ถ้ามีข้อมูลการตรวจนับ
	{
		...
		foreach($re as $rs)
		{
			...
			$this->excel->getActiveSheet()->setCellValue("C".$row, $rs->item_code);
			$this->excel->getActiveSheet()->setCellValue("D".$row, $rs->item_name);
			$this->excel->getActiveSheet()->setCellValue("E".$row, $rs->cost);
			$this->excel->getActiveSheet()->setCellValue("F".$row, $rs->price);
			$this->excel->getActiveSheet()->setCellValue("G".$row, $rs->instock);
		...
		$imported = $this->report_model->get_instock_but_not_check($id);
		...
		$this->excel->getActiveSheet()->getStyle("A".$row.":M".$row)->getBorders()->getBottom()->setBorderStyle(PHPExcel_Style_Border::BORDER_DOUBLE);
